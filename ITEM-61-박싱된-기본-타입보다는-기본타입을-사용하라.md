...
책에서는 이렇게 이야기하지만, 내 생각은 박싱된 기본 타입을 사용하는 것이 이점이 더 많은 것 같다는 생각이 든다. 기본 타입은 `null` 과 같은 초기화되지 않은 값을 표현할 방법이 마땅치 않다. `null` 처리를 잘 해두었다면 오히려 박싱된 기본타입이 더 편할때도 있지 않을까 싶다.<br>

또한 제너릭 등을 사용하는 기능을 구현할 때에는 기본타입을 사용하지 못하는 단점이 있다. 이런 이유로 개인적으로는 기본 타입보다는 박싱된 기본 타입이 더 좋은 방식이 아닐까 하고 생각이 든다.<br>

한 가지 유의해야 할 점은 아무리 주의해도 실수를 할때가 있는 경우다. `==` 연산으로 값의 동치성 검사를 수행하도록 코드를 작성하게 되는 경우가 있다. 아무리 주의를 해도, 변수가 작게 보이고, 여러 일들이 많으면 주의를 할수 없게 된다. 기본 타입에 대해서는 `==` 연산이 값의 비교가 잘 이뤄지지만, 참조형 타입간의 `==` 연산은 객체의 식별값을 비교하기에 전혀 다른 결과를 낸다. 평상시에 테스트 코드 기반으로 개발하는 습관이 잘 들여져 있다면, 이런 경우에 개발 단계에서 미리 충분히 대응이 가능할 것으로 보인다.<br>
